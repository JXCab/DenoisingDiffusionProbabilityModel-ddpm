...
class DiagonalGaussianDistribution(object):
    ...
    def __init__(self, parameters, deterministic=False):
        ...
        self.mean, self.logvar = torch.chunk(parameters, 2, dim=1) # 将张量parameters分为两个部分(从dim=1), 并赋值给均值和对数方差
        self.logvar = torch.clamp(self.logvar, -30.0, 20.0)        # 将对数方差限制在(-30.0, 20.0)这个范围
        self.deterministic = deterministic
        self.std = torch.exp(0.5 * self.logvar)  # 对数方差 --> 标准差
        self.var = torch.exp(self.logvar)        # 对数方差 --> 方差   var=std*std
